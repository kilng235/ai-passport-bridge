...
- 对讲机语音链路端到端可靠性加固。设备端：语音任务现在自行初始化 ES8311（此前依赖
  「先响过一次提示音」才有 codec，刚重启就录音会报「麦克风初始化失败」）；提示音任务
  在录音期间完全避让音频硬件；新增 ~400ms 预热丢弃 ADC 启动瞬态（巨大直流偏移 + 削波，
  此前导致 ASR 随机返回空文本）；有效性判定改为按实际流式字节数（≥0.3 秒）而非墙钟时间。
  PC 端：识别为空时不再向会话注入占位 Prompt，commit 端点返回 422，设备明确提示失败供重录。
